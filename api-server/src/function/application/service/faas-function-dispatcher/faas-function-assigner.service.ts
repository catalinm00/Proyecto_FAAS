...
} from '../../../infrastructure/database/mongo-faasfunction-execution-repository';
import {
  FunctionExecutionRequestedPublisher,
} from '../../../infrastructure/messaging/function-execution-requested-publisher';
import {
...
export class FaasFunctionAssignerService {
  constructor(private readonly functionExecutionPublisher: FunctionExecutionRequestedPublisher,
  ...
  async getResult(execution: FaasFunctionExecution): Promise<FaasFunctionExecutionResult> {
    return new Promise(async (resolve, reject) => {
      let subscription = this.functionResponseSubscriber.subscribe(execution.id);
